...
cp ${SCRIPT_DIR}/terraform/elasticache-subnet-group.tf ${TMP_DIR}
...
SUBNET_GROUP=$(terraform ${TMP_DIR} output -json name | jq -r .)
VPC_SECURITY_GROUP_ID=$(terraform ${TMP_DIR} output -json security_group_id | jq -r .)

echo "{\"elasticache_subnet_group\" : \"${SUBNET_GROUP}\", \"elasticache_vpc_security_group_ids\" : \"${VPC_SECURITY_GROUP_ID}\"}"
