...
func NewTerraformMock(opts ...Option) (TerraformMock, error) {
	...
	build, err := gexec.Build("github.gwd.broadcom.net/TNZ/cloud-service-broker/v2/brokerpaktestframework/mock-binary/tofu", "-ldflags", fmt.Sprintf("-X 'main.InvocationStore=%s'", dir))
	if err != nil {
		return TerraformMock{}, err
	}
