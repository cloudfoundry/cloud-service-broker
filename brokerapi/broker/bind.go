...
func (broker *ServiceBroker) Bind(ctx context.Context, instanceID, bindingID string, details domain.BindDetails, _ bool) (domain.Binding, error) {
	...
	binding.Credentials, err = broker.credStore.Save(computeCredHubPath(broker.getServiceName(serviceDefinition), bindingID), binding.Credentials, parsedDetails.CredHubActor)
	if err != nil {
