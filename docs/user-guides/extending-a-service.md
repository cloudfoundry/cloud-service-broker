...
In this tutorial, we are going to add a feature to an existing service in
the [AWS Brokerpak](https://github.com/cloudfoundry/csb-brokerpak-aws) for the S3 service. Our goal is to be able to add
a feature to the service definition, write the accompanying OpenTofu language, and add adequate testing so that a PR can be
raised against the Brokerpak.
...
Before you start, you should be familiar with Brokerpak concepts, for example by reading
the [writing your first brokerpak](./writing-your-first-brokerpak.md) tutorial.
...
Before thinking about adding a feature to a service, it's important to be able to understand a service definition and
identify if the feature is already exposed.

In our use case we're using the AWS S3 service and the feature we want to enable on our bucket is for the requester to
pay for requests to the bucket.

First we navigate to the service definition file `aws-s3-bucket.yml`. By convention, these files exist at the top level
of the Brokerpak directory and are defined in YAML.

Given that we're interested in which properties can be defined when provisioning a service we can navigate within the
file to the `provision` block:
...
We want the ability to configure the bucket so that the requester pays, but there are no existing fields related to this
property, so we're going to have to add one.

## Identifying a feature you want to add

We have determined that there are no exposed properties to allow us to configure the requester to pay. However, it may
be possible that this field is actually defined in the OpenTofu language for this service, it just hasn't been exposed as a
configurable property.

To understand this we need to look at the `main.tf` OpenTofu language file for the S3 service. Within the CSB-Brokerpak-AWS
...
To understand this file we need a bit more context on the Terraform provider being used, so we can understand how the
service is being created.

We can tell by looking at the `provider.tf` file that we are using the AWS Terraform provider. Looking at
the [Terraform provider documentation](https://registry.terraform.io/providers/hashicorp/aws/latest/docs) will help us
determine how to configure the bucket so that the requester pays.
...
that in order for the requester to pay, it's necessary to create a resource of
...
test will check that the OpenTofu language works, and that the default value for
...
broker are correctly translated into OpenTofu language variables. The tests are written in Go using
...
This ensures that the OpenTofu language variable takes the correct default value. You can examine other tests
...
OpenTofu tests are `unit` tests that run with the system's **installed** OpenTofu binary on the OpenTofu language files
directly.
...
OpenTofu sends the expected values to the providers. The language used to write the tests is Go and
...
the logic applied in your OpenTofu code in which ternary operations or local variables are usually added. Thanks to
this type of test, you add an extra layer of security for future changes.

As you see in the next section, you can check that the new resource added will be created with the
expected values:
...
To run the tests, run `make run-terraform-tests` in the top level of the brokerpak, but before doing it,
...
Two environment variables are mandatory on AWS: `AWS_SECRET_ACCESS_KEY` and `AWS_ACCESS_KEY_ID`, both used to provide
the credentials.
