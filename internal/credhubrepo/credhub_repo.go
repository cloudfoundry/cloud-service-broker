...
func (s *Repo) Save(path string, cred any, actor string) (any, error) {
	...
	if err := s.http(http.MethodPut, "/api/v1/data", setRequestBody, nil, http.StatusOK); err != nil {
	...
	if err := s.http(http.MethodPost, "/api/v2/permissions", permsRequestBody, nil, http.StatusCreated); err != nil {
...
func (s *Repo) Delete(path string) error {
	...
	if err := s.http(http.MethodGet, fmt.Sprintf("/api/v1/permissions?credential_name=%s", path), nil, &listPermissionsResponseBody, http.StatusOK); err != nil {
	...
	for _, p := range listPermissionsResponseBody.Permissions {
		...
		if err := s.http(http.MethodGet, fmt.Sprintf("/api/v2/permissions?%s", query), nil, &getPermissionResponseBody, http.StatusOK); err != nil {
			return fmt.Errorf("failed to get permission %q for credential %q: %w", p.Actor, path, err)
		}

		if err := s.http(http.MethodDelete, fmt.Sprintf("/api/v2/permissions/%s", getPermissionResponseBody.UUID), nil, nil, http.StatusOK); err != nil {
			return fmt.Errorf("failed to delete permission ID %q: %w", getPermissionResponseBody.UUID, err)
		}
	}

	// Delete the credential
	if err := s.http(http.MethodDelete, fmt.Sprintf("/api/v1/data?name=%s", path), nil, nil, http.StatusNoContent); err != nil {
